...
from config_shared import (
    ...
    IMAGE_SAMPLES_DIR, CSV_REPORT_FIELDS
)
...
def compute_boundary_metrics_from_coco(
    ...
    try:
        ...
        # Build GT lookup: image_id -> list of (gt_mask_np, category_id, ann_id)
        coco_obj = COCO()
        ...
        # Greedy matching: urutkan prediksi dari skor tertinggi
        used_gt: dict = {}
        biou_scores = []
    ...
    for img_path in subset:
        ...
        if mtype in ["yolo_det", "yolo_seg"]:
            ...
            if res.boxes is not None and len(res.boxes) > 0:
                ...
                # 1. SAM2/MobileSAM Inference — STANDARDIZED: Menggunakan Tensor GPU Mentah (res.boxes.xyxy)
                torch.cuda.synchronize()
    ...
    if not args.skip_eval:
        ...
        for mcfg in MODELS_CONFIG:
            ...
            try:
                ...
                mAP50_box, mAP95_box, recall_box, precision_box = "N/A", "N/A", "N/A", "N/A"
                mAP50_mask, mAP95_mask, recall_mask, precision_mask = "N/A", "N/A", "N/A", "N/A"
                ...
                if len(dt_segm) > 0 and (mtype in ["yolo_seg", "maskrcnn", "hybrid_seg", "hybrid_seg_mobile", "hybrid_det", "hybrid_det_mobile"]):
                    try:
                        ...
                        coco_gt_filtered = copy.deepcopy(coco_gt)
                        valid_anns = []
                        for ann in coco_gt_filtered.get("annotations", []):
                            seg = ann.get("segmentation")
                            if seg and (isinstance(seg, dict) or (isinstance(seg, list) and len(seg) > 0)):
                                valid_anns.append(ann)
                        coco_gt_filtered["annotations"] = valid_anns

                        coco_gt_obj = COCO()
                        coco_gt_obj.dataset = coco_gt_filtered
                        coco_gt_obj.createIndex()
        ...
        comp_gold_csv = os.path.join(PAPER1_CSV_DIR, "kompilasi_ALL_new_method_golden.csv")
        ...
        send_telegram_msg(
            ...
            f"Kompilasi: <code>kompilasi_ALL_new_method_golden.csv</code>\n"
...
""")
        ...
        if seg_rows_with_boundary:
            print("\n  Ringkasan Boundary Metrics Golden (Model Segmentasi):")
