# -*- coding: utf-8 -*-
"""
utils/golden_evaluation.py
===========================
Golden Pixel-Perfect Evaluation — Distributed Multi-GPU.

Mengevaluasi seluruh pipeline (YOLO Det, YOLO Seg, Mask R-CNN, Hybrid)
terhadap Golden Pixel-Perfect Test Set untuk mengekspos degradasi akurasi
model end-to-end ketika dihadapkan dengan ground truth absolut.

Cara pakai:
  # Mode default (otomatis memuat golden_dataset_det & golden_dataset_seg)
  tmux new-session -d -s golden_eval "cd Trainning-Models/MyFineTunning-dev && source /data/programs/anaconda3/bin/activate && conda activate yolo_env && python3 -u utils/golden_evaluation.py 2>&1 | tee utils/golden_evaluation.log"

  # Manual override
  python3 utils/golden_evaluation.py --dataset /path/to/dataset --coco

  # Default dari path manapun
    tmux new-session -d -s golden_evaluation "cd /root/Trainning-Models/MyFineTunning-RunPOD && python3 -u utils/golden_evaluation.py 2>&1 | tee utils/golden_evaluation.log"
"""
...
from config_shared import (
    WORKSPACE_DIR, IMAGE_SIZE, NUM_CLASSES,
    get_output_dir, REPORTS_DIR, DATA_FILES_DIR,
    GOLDEN_DET_DATASET_LOCATION, GOLDEN_SEG_DATASET_LOCATION, PAPER1_CSV_DIR
)
...
if __name__ == "__main__":
    ...
    if args.dataset == "default":
        print("\n[Setup] Mode Auto: Memuat Golden Dataset Deteksi & Segmentasi secara terpisah...\n")
    ...
    print("=" * 65)
    print(f"  All Models — Golden Pixel-Perfect Evaluation [{suffix}]")
    ...
    if all_det_rows:
        det_csv = os.path.join(PAPER1_CSV_DIR, "golden_det.csv")
        ...
        print(f"\n✅ Golden Det Report: {det_csv}")

    if all_seg_rows:
        seg_csv = os.path.join(PAPER1_CSV_DIR, "golden_seg.csv")
        ...
        print(f"✅ Golden Seg Report: {seg_csv}")
