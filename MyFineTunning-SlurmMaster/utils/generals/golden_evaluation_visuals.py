# -*- coding: utf-8 -*-
"""
utils/golden_evaluation_visuals.py
====================================
Visualisasi bukti konsep untuk evaluasi Golden Pixel-Perfect Dataset.

Menggunakan dataset yang sama persis dengan golden_evaluation.py:
  - GOLDEN_DET_DATASET_LOCATION (golden_dataset_det)
...
# ====================================================================
# Konfigurasi Direktori — Golden Dataset
# ====================================================================
...
GOLD_DET_DIR = _resolve_valid_dir(GOLDEN_DET_DATASET_LOCATION)
GOLD_SEG_DIR = _resolve_valid_dir(GOLDEN_SEG_DATASET_LOCATION)

# Output Directory — paper1/visuals/golden/
GOLD_VIS_DIR   = os.path.join(PAPER1_VIS_DIR, "golden")
...
def main():
    print(f"\n{'='*65}")
    print(f"  Golden Pixel-Perfect Evaluation Visuals")
    print(f"  Det Dataset: {GOLD_DET_DIR}")
    print(f"  Seg Dataset: {GOLD_SEG_DIR}")
    ...
    init_classes_from_coco(GOLD_SEG_DIR)
    ...
    # 3. Load GT
    coco_det = load_gt_annotations(GOLD_DET_DIR)
    coco_seg = load_gt_annotations(GOLD_SEG_DIR)

    # Ambil daftar gambar dari SEG dataset
    img_names = sorted([f for f in os.listdir(GOLD_SEG_DIR) if f.lower().endswith(('.jpg', '.png', '.jpeg'))])
    print(f"Total gambar ditemukan: {len(img_names)}")

    for idx, img_name in enumerate(img_names):
        print(f"[{idx+1}/{len(img_names)}] Memproses {img_name}...")
        img_path = os.path.join(GOLD_SEG_DIR, img_name)
    ...
    print(f"\n✅ Golden Evaluation Visuals selesai.")
