...
if not args.skip_eval:
    print("\n" + "="*65 + "\n  Menjalankan Evaluasi YOLOv10 (m, x) via generate_report_single_model.py\n" + "="*65)
    import subprocess
    ...
    if not os.path.exists(eval_script):
        ...
        try:
            subprocess.run(
                [sys.executable, "-u", eval_script, "--family", "yolov10", "--gpus", str(eval_gpu)],
                check=True,
            )
        except subprocess.CalledProcessError as e:
            print(f"\n⚠️ Evaluasi YOLOv10 gagal: {e}")
else:
