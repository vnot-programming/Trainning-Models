...
if not args.skip_eval:
    print("\n" + "="*65 + "\n  Menjalankan Evaluasi YOLO11 (n, n-seg, l, l-seg, x, x-seg) via generate_report_single_model.py\n" + "="*65)
    import subprocess
    ...
    if not os.path.exists(eval_script):
        ...
        try:
            subprocess.run(
                [sys.executable, "-u", eval_script, "--family", "yolo11", "--gpus", str(eval_gpu)],
                check=True,
            )
        except subprocess.CalledProcessError as e:
            print(f"\n⚠️ Evaluasi YOLO11 gagal: {e}")
else:
