...
def load_native_coco_gt(valid_dir):
    """
    Load COCO ground truth dari format asli COCO JSON (_annotations.coco.json).
    Returns:
        coco_gt_dict: dict dalam format COCO
        image_ids: dict {image_path: coco_image_id}
    """
    coco_json = os.path.join(valid_dir, "_annotations.coco.json")
    if not os.path.exists(coco_json):
        print(f"[COCO] ⚠️ JSON COCO tidak ditemukan di: {coco_json}")
        return None, None
    with open(coco_json, 'r') as f:
        coco_gt_dict = json.load(f)
    image_ids = {img["file_name"]: img["id"] for img in coco_gt_dict.get("images", [])}
    print(f"[COCO] ✅ Native Ground truth dimuat dari {coco_json}: {len(coco_gt_dict.get('images', []))} images, {len(coco_gt_dict.get('annotations', []))} annotations")
...
def evaluate_coco_predictions(coco_gt_dict, image_ids, predictions, iou_type="bbox"):
    ...
    if not coco_preds:
        print(f"[COCO] ⚠️ Tidak ada prediksi yang cocok untuk dievaluasi ({iou_type}).")
        return 0.0, 0.0
