...
from config_shared import (
    ...
    DATASETS_DIR, MODELS_DIR, REPORTS_DIR, PAPER1_CSV_DIR, VISUALS_DIR, IMAGE_SAMPLES_DIR,
...
try:
    ...
    # Fallback sementara: jika dataset fine-tuning dinonaktifkan, gunakan nilai lama
    DET_DATASET_LOCATION = _datasets.get("det_location", DET_DATASET_LOCATION)
    SEG_DATASET_LOCATION = _datasets.get("seg_location", SEG_DATASET_LOCATION)
    DET_YAML             = _datasets.get("det_yaml", DET_YAML)
    SEG_YAML             = _datasets.get("seg_yaml", SEG_YAML)
    
    # Pengecekan Ketersediaan Golden & Standard Datasets
    golden_seg = _datasets.get("coco_seg_location")
    golden_det = _datasets.get("coco_det_location")
    std_det    = _datasets.get("h61lr_det_yaml")
    std_seg    = _datasets.get("h61lr_seg_yaml")
    
    if not all([golden_seg, golden_det, std_det, std_seg]):
        raise RuntimeError("Incomplete Dataset: Golden Dataset atau Standard Dataset gagal diinisialisasi.")
        
    print(f"[Setup] ✅ Golden Seg: {golden_seg}")
