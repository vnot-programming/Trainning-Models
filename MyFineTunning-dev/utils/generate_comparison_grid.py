...
# CONSTANTS
# ==============================================================================
UNU_VALID_DIR  = os.path.join(DATASETS_DIR,
                              "standard_datasets_seg", "valid")
...
def generate(device_str: str = "cuda:0"):
    ...
    with open(UNU_ANNOTATIONS) as f:
        coco_data = json.load(f)
    cats = [c for c in coco_data.get('categories', []) if c['id'] > 0]
    cats = sorted(cats, key=lambda x: x['id'])
    CLASS_NAMES = [c['name'] for c in cats]
    
    # Gunakan image sample dari standard evaluation visuals
    IMG_SAMPLE_DIR = os.path.join(PAPER1_VIS_DIR, "standard", "images_sample")
    img_files = sorted(glob.glob(os.path.join(IMG_SAMPLE_DIR, "*.*")))
