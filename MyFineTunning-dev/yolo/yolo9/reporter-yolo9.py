# -*- coding: utf-8 -*-
"""
yolo/yolo9/reporter-yolo9.py
=============================
Standalone evaluasi & laporan untuk YOLOv9m Detection + YOLOv9c-Seg.
Format CSV identik dengan MyTrainEngine/evaluation/reporter.py.

Output:
  - runs/reports/report_yolov9m_det.csv   → format det_performance
  - runs/reports/report_yolov9c_seg.csv   → format seg_performance

Workspace : /workspace/MyFineTunning-20260423_104653
best.pt   : runs/yolov9m/weights/best.pt
            runs/yolov9c_seg/weights/best.pt

Cara menjalankan:
    python -u reporter-yolo9.py 2>&1 | tee yolov9_reporter.log
"""
...
parser = argparse.ArgumentParser(description="YOLOv9 Reporter")
...
best_seg = os.path.join(get_output_dir("yolov9c_seg"), "weights", "best.pt")
...
if os.path.exists(best_det):
    ...
    det_row = {"Model": "YOLOv9m (Fine-tuned)",
...
det_fields = ["Model", "Model Size (MB)", "mAP50-95", "mAP50",
              "Precision", "Recall",
              "Preprocess (ms)", "Inference (ms)", "Postprocess (ms)"]
det_csv = os.path.join(report_dir, "report_yolov9m_det.csv")
...
if os.path.exists(best_seg):
    ...
    seg_row = {"Model": "YOLOv9c-Seg (Fine-tuned)",
...
print("\n" + "="*60)
print("  [3/3] Complexity Measurement — YOLOv9")
print("="*60)

cplx_rows = []
for model_label, pt_path, arch_type in [
    ("YOLOv9m (Fine-tuned)",      best_det, "Object Detection"),
    ("YOLOv9c-Seg (Fine-tuned)",  best_seg, "End-to-End Segmentation"),
]:
    print(f"  ▶ {model_label}")
...
cplx_csv = os.path.join(report_dir, "report_yolov9_complexity.csv")
